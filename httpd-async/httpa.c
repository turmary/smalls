...
BOOL StartHTTP(LPHTTPSERVINFO lpInfo)
{
	...
	if (nRet == SOCKET_ERROR) {
		LogWinSockError(ghwnd, "WSAAsyncSelect() error", WSAGetLastError());
...
void OnAccept(HWND hwnd, SOCKET socket, int nErrorCode)
{
	...
	if (peerSocket == SOCKET_ERROR) {
		...
		if (nRet != WSAEWOULDBLOCK) {
			...
			//
			LogWinSockError(ghwnd, "accept() error", WSAGetLastError());
			return;
		}
	}
	//
	...
	LogEvent(ghwnd, "Connection accepted on socket %d from: %s", peerSocket, inet_ntoa(SockAddr.sin_addr));
}
...
void OnRead(SOCKET socket, int nErrorCode)
{
	...
	if (nRet == SOCKET_ERROR) {
		if (WSAGetLastError() == WSAEWOULDBLOCK)
			return;
		LogWinSockError(ghwnd, "recv()", WSAGetLastError());
...
void SendFileContents(LPREQUEST lpReq)
{
	...
	while (1) {
		...
		if (uBytes == HFILE_ERROR) {
			LogEvent(ghwnd, "Read file error: %d", GetLastError());
		...
		//
		nBytesSent = send(lpReq->Socket, buf, uBytes, 0);
		if (nBytesSent == SOCKET_ERROR) {
			if (WSAGetLastError() != WSAEWOULDBLOCK) {
				LogWinSockError(ghwnd, "send()", WSAGetLastError());
				CloseConnection(lpReq);
				_lclose(lpReq->hFile);
				lpReq->hFile = HFILE_ERROR;
				return;
