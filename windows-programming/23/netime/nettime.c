...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow)
{
	...
	hwnd =
	    CreateWindow(szAppName, TEXT("Set System Clock from Internet"),
			 WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU |
			 WS_BORDER | WS_MINIMIZEBOX, CW_USEDEFAULT,
...
{
	static char szIPAddr[32] = { "128.138.140.44" };
...
void FormatUpdatedTime(HWND hwndEdit, SYSTEMTIME * pstOld, SYSTEMTIME * pstNew)
{
	...
	GetDateFormat(LOCALE_USER_DEFAULT,
		      LOCALE_NOUSEROVERRIDE | DATE_SHORTDATE, pstOld, NULL, szDateOld, sizeof(szDateOld));
	GetTimeFormat(LOCALE_USER_DEFAULT,
		      LOCALE_NOUSEROVERRIDE | TIME_NOTIMEMARKER |
		      TIME_FORCE24HOURFORMAT, pstOld, NULL, szTimeOld, sizeof(szTimeOld));

	GetDateFormat(LOCALE_USER_DEFAULT,
		      LOCALE_NOUSEROVERRIDE | DATE_SHORTDATE, pstNew, NULL, szDateNew, sizeof(szDateNew));
	GetTimeFormat(LOCALE_USER_DEFAULT,
		      LOCALE_NOUSEROVERRIDE | TIME_NOTIMEMARKER |
	...
	EditPrintf(hwndEdit, TEXT("System date and time successfully changed ")
		   TEXT("from\r\n\t%s, %s.%03i to\r\n\t%s, %s.%03i."),
		   szDateOld, szTimeOld, pstOld->wMilliseconds, szDateNew, szTimeNew, pstNew->wMilliseconds);
}
