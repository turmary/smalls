...
BOOL StartHTTP(LPHTTPSERVINFO lpInfo)
{
	...
	if (listenSocket == INVALID_SOCKET) {
		LogWinSockError(ghwnd, "Could not create listen socket", WSAGetLastError());
		return FALSE;
	}
	//
	...
	if (nRet == SOCKET_ERROR) {
		LogWinSockError(ghwnd, "bind() error", WSAGetLastError());
	...
	if (nRet == SOCKET_ERROR) {
		LogWinSockError(ghwnd, "listen() error", WSAGetLastError());
		closesocket(listenSocket);
		return FALSE;
	}
	//
	...
	//
	gdwListenThread = _beginthreadex(NULL,		// Security
					 0,		// Stack size
	...
	if (!gdwListenThread) {
		LogEvent(ghwnd, "Could not create listening thread: %d", GetLastError());
		closesocket(listenSocket);
		return FALSE;
	}
	//
	...
	LogEvent(ghwnd, "HTTP Server Started: %s [%s] on port %d", szBuf, szAddress, htons(saServer.sin_port));
...
unsigned __stdcall ListenThread(void *pVoid)
{
	...
	while (1) {
		...
		socketClient = accept(listenSocket, (LPSOCKADDR) & SockAddr, &nLen);
		...
		// 
		LogEvent(ghwnd, "Connection accepted on socket:%d from:%s", socketClient, inet_ntoa(SockAddr.sin_addr));
		...
		if (lpReq == NULL) {
			LogEvent(ghwnd, "No memory for client request");
			continue;
		}
		lpReq->hExit = *pHandle;
		...
		//
		dwClientThread = _beginthreadex(NULL,		// Security
						0,		// Stack size
	...
	if (dwRet == WAIT_TIMEOUT) {
		LogEvent(ghwnd, "One or more client threads did not exit");
	}
	DeleteClientCount();
	return 0;
}
...
BOOL RecvRequest(LPREQUEST lpReq, LPBYTE pBuf, DWORD dwBufSize)
{
	...
	if (nRet != 0) {
		if (WSAGetLastError() != WSA_IO_PENDING) {
			LogWinSockError(ghwnd, "WSARecv()", WSAGetLastError());
	...
	if (fPending) {
		...
		if (!WSAGetOverlappedResult(lpReq->Socket, &over, &dwRecv, FALSE, &dwFlags)) {
			CloseHandle(over.hEvent);
			return FALSE;
		}
	}
...
void CloseConnection(LPREQUEST lpReq)
{
	...
	//
	LogEvent(ghwnd, "Closing socket: %d", lpReq->Socket);

	nRet = closesocket(lpReq->Socket);
	if (nRet == SOCKET_ERROR) {
		LogWinSockError(ghwnd, "closesocket()", WSAGetLastError());
	}
	//
	...
	SendMessage(ghwnd, guAppMsg, HTTP_STATS_MSG, (LPARAM) & stats);
}
...
void SendFile(LPREQUEST lpReq)
{
	...
	if (lpReq->hFile == INVALID_HANDLE_VALUE) {
		SendMessage(ghwnd, guAppMsg, HTTP_FILENOTFOUND_MSG, (LPARAM) (LPCSTR) lpReq->szFileName);
	...
	//
	SendMessage(ghwnd, guAppMsg, HTTP_FILEOK_MSG, (LPARAM) (LPCSTR) & lpReq->szFileName[strlen(szWebRoot)]);
	//
	...
	if (CloseHandle(lpReq->hFile))
		...
		LogEvent(ghwnd, "Error closing file: %d", GetLastError());
}
...
void SendFileContents(LPREQUEST lpReq)
{
	...
	while (1) {
		...
		//
		fRet = ReadFile(lpReq->hFile, buf, sizeof(buf), &dwRead, NULL);
		if (!fRet) {
			// Send "404 Not Found" error
...
BOOL SendBuffer(LPREQUEST lpReq, LPBYTE pBuf, DWORD dwBufSize)
{
	...
	if (nRet != 0) {
		if (WSAGetLastError() == WSA_IO_PENDING)
			...
			LogWinSockError(ghwnd, "WSASend()", WSAGetLastError());
			CloseHandle(over.hEvent);
			return FALSE;
		}
	}
	...
	if (fPending) {
		...
		//
		if (!WSAGetOverlappedResult(lpReq->Socket, &over, &dwRecv, FALSE, &dwFlags)) {
			LogWinSockError(ghwnd, "WSAGetOverlappedResult()", WSAGetLastError());
			CloseHandle(over.hEvent);
			return FALSE;
		}
	}
...
#define NUMSTATMSGS sizeof(szStatMsgs) / sizeof(szStatMsgs[0])
	...
	send(lpReq->Socket, szMsg, strlen(szMsg), 0);
}
...
void LogEvent(HWND hwnd, LPCSTR lpFormat, ...)
{
	...
	va_end(Marker);
	SendMessage(ghwnd, guAppMsg, HTTP_EVENT_MSG, (LPARAM) szBuf);
}
