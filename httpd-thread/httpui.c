...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (nRet) {
		MessageBox(NULL, "Initialize WinSock Failed", gszAppName, MB_OK);
		return 1;
	}
	// Check version
	if (wsaData.wVersion != wVersionRequested) {
		MessageBox(NULL, "Wrong WinSock Version", gszAppName, MB_OK);
		return 2;
	}
	//
...
BOOL OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
	...
	//
	SetClassLong(hwnd, GCL_HICON, (LONG) LoadIcon(GetWindowInstance(hwnd), MAKEINTRESOURCE(IDI_APP)));
...
void OnCommand(HWND hwnd, int nId, HWND hwndCtl, UINT codeNotify)
{
	...
	case IDC_START_STOP:
		...
		if (!fStarted) {
			...
			//
			nRet = DialogBox(GetWindowInstance(hwnd), MAKEINTRESOURCE(IDD_SERVINFO), hwnd, InfoWndProc);
			if (nRet == IDOK) {
				// Fill in the server info
				si.nPort = GetProfileInt(gszAppName, gszPortKey, 0);
				GetProfileString(gszAppName, gszDirKey, "", szRootDir, sizeof(szRootDir));
...
void FileRequested(HWND hwnd, int nError, LPCSTR lpFileName)
{
	...
	if (!nError)
		...
		wsprintf(szDisplayName, "%d - %s", nError, lpFileName);
...
void ShowEvent(HWND hwnd, LPCSTR lpEvent)
{
	...
	GetLocalTime(&st);
	wsprintf(szBuf, "%02d:%02d:%02d.%03d\t%s", st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, lpEvent);
	ListBox_AddString(hwndEvents, szBuf);
	if (ListBox_GetCount(hwndEvents) > 500)
		ListBox_DeleteString(hwndEvents, 0);
	ListBox_SetCurSel(hwndEvents, ListBox_GetCount(hwndEvents) - 1);
}

////////////////////////////////////////////////////////////

BOOL CALLBACK InfoWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	char szText[256];
	BOOL fRet = 0;
	switch (uMsg) {
	case WM_INITDIALOG:
		...
		//
		GetProfileString(gszAppName, gszPortKey, "", szText, sizeof(szText));
		SetDlgItemText(hwnd, IDC_PORT, szText);

		GetProfileString(gszAppName, gszDirKey, "", szText, sizeof(szText));
		SetDlgItemText(hwnd, IDC_ROOTDIR, szText);
		fRet = TRUE;
	case WM_COMMAND:
		switch (wParam) {
		case IDOK:
			...
			//
			GetDlgItemText(hwnd, IDC_PORT, szText, sizeof(szText));
			WriteProfileString(gszAppName, gszPortKey, szText);

			GetDlgItemText(hwnd, IDC_ROOTDIR, szText, sizeof(szText));
			WriteProfileString(gszAppName, gszDirKey, szText);
			fRet = TRUE;
			EndDialog(hwnd, IDOK);
			break;
		case IDCANCEL:
