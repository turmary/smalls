...
char *where;
{
	return build_1hex_number((opcode_pt) num, build_1hex_number((opcode_pt) (num / 256), where));
}
...
register char *where;
{
	...
	do {
		for (digit = '0', power_of_10 = (powers_of_10 - 1)[power]; num >= power_of_10; num -= power_of_10)
			++digit;
		if (power <= width)
			*where++ = digit;
	}
...
char *err_str;
{
	...
	if (errcount >= MAXERR)
		...
		for (errptr = errbuf + errcount; errptr > errbuf && errptr->position > position; errptr = errptrlow) {
			errptrlow = errptr - 1;
...
PRIVATE void listcode()
{
	...
	if (popflags & POPLO) {
#if SIZEOF_OFFSET_T > 2
		if (popflags & POPLONG)
			build_2hex_number((u16_T) (lastexp.offset / (offset_t) 0x10000L), listptr->displ4);
#endif
		if (popflags & POPHI)
		...
		if (count > 1)
			build_1hex_number((opcode_pt) (lastexp.offset >> 0x8), listptr->displ2);
#if SIZEOF_OFFSET_T > 2
		if (count > 2) {
			build_1hex_number((opcode_pt) (lastexp.offset >> 0x10), listptr->displ3);
			build_1hex_number((opcode_pt) (lastexp.offset >> 0x18), listptr->displ4);
		}
#endif
#ifdef I80386
		if (immcount > 0) {
		...
		if (immcount > 1)
			build_1hex_number((opcode_pt) (immadr.offset >> 0x8), listptr->imm2);
		if (immcount > 2) {
			build_1hex_number((opcode_pt) (immadr.offset >> 0x10), listptr->imm3);
			build_1hex_number((opcode_pt) (immadr.offset >> 0x18), listptr->imm4);
		}
#endif
	}
	writes((char *) listptr);
