...
FORWARD void sort P((struct sym_s ** array, struct sym_s ** top, bool_pt nameflag));
...
unsigned column;
{
	...
	build_2hex_number((unsigned) (symptr->value_reg_or_op.value >> 16), listptr->value);
#endif
	build_2hex_number((unsigned) symptr->value_reg_or_op.value, listptr->value);
	writes((char *) listptr);
			...
			for (i = j - gap; i >= 0; i -= gap) {
				...
				if ((bool_t) nameflag) {
					if (strcmp((*left)->name, (*right)->name) <= 0)
						break;
				} else if ((unsigned) (*left)->value_reg_or_op.value <= (*right)->value_reg_or_op.value)
					break;
				swap = *left;
				*left = *right;
				*right = swap;
