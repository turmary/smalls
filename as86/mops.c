	...
	if (eap->indcount == 0x0)
		...
		if (eap->base == NOREG) {
			...
			if (eap->base > MAX16BITINDREG) {
				...
				asize = 0x2;
				if (!(lastexp.data & UNDBIT) && !iswordorswordoffset(lastexp.offset))
					error(ABOUNDS);
				if (eap->index == NOREG)
		...
		if (eap->base == NOREG)
			mcount += asize;
		else if (lastexp.data & (FORBIT | RELBIT | UNDBIT) || !is8bitsignedoffset(lastexp.offset)) {
			postb |= MEM2_MOD;
			mcount += asize;
		} else if (lastexp.offset != 0x0 || (eap->base == BPREG && eap->index == NOREG) || eap->base == EBPREG) {
			postb |= MEM1_MOD;
			++mcount;
		}
		if (asize > 0x2 && (eap->base == ESPREG || eap->index != NOREG)) {
			sib = ss[eap->scale] | (rm[eap->index] << INDEX_SHIFT) | (postb & RM_MASK);
...
register struct ea_s *eap;
{
	...
	if (!(eap->displ.data & UNDBIT)) {
		if (asize > 0x2) {
			if (!(eap->displ.data & (FORBIT | RELBIT)) && iswordadr(eap->displ.offset))
				asize = 0x2;
...
register struct ea_s *eap;
{
	if ((eap->base = regchk()) != AXREG && eap->base != ALREG && eap->base != EAXREG)
		return FALSE;
	getsym();
...
register struct ea_s *eap;
{
	...
	leading_immed = leading_displ = lastexp.data = eap->indcount = lastexp.offset = 0x0;
			...
			if (symptr->data & SIZEBIT) {
				...
				if (symptr->value_reg_or_op.op.opcode == 0x0)
					...
					if (sym == IDENT &&
					    ...
					    symptr->data & SIZEBIT && symptr->value_reg_or_op.op.routine == PTROP) {
						getsym();
						eap->indcount = 0x2 - calljmp_kludge;
					}
				}
		...
		if ((eap->base = indregchk((reg_pt) NOREG)) != NOREG) {
			...
			if (sym == STAR) {
				...
				if (!(lastexp.data & UNDBIT) && lastexp.offset != 0x1) {
					if (eap->base <= MAX16BITINDREG ||
					    (lastexp.offset != 0x2 && lastexp.offset != 0x4 && lastexp.offset != 0x8))
						error(ILL_SCALE);
					else {
	...
	if (pass2) {
		...
		if (!(lastexp.data & (RELBIT | UNDBIT))) {
			...
			if (last_pass < 2 && backamount != 0x0 &&
			    !(lastexp.data & IMPBIT) && lastexp.offset + backamount < 0x80 + backamount)
...
PUBLIC void mcall()
{
	...
	far_diff = 0x0;
	if (sym == IDENT && (symptr = gsymptr)->type & MNREGBIT && symptr->data & SIZEBIT) {
		if (symptr->value_reg_or_op.op.routine == FAROP) {
			far_diff = 0x8;
			getsym();
		}
		if (symptr->value_reg_or_op.op.routine == WORDOP && opcode == JMP_SHORT_OPCODE) {
			opcode = JMP_OPCODE;
			getsym();
		}
	}
	...
	if (sym == COLON) {
		...
		if (mcount != 0x0) {
			...
			if (!(lastexp.data & (FORBIT | RELBIT | UNDBIT)) &&
			    tsize == 0x2 && (offset_t) (lastexp.offset + 0x8000L) >= 0x18000L)
				datatoobig();
			mcount += tsize;
			target.size = 0x2;
		...
		if (jumps_long && ((pass != 0 && !is8bitsignedoffset(lastexp.offset - lc - 2)) || (last_pass == 1))) {
			opcode = JMP_OPCODE;
			lbranch(0x83);
		} else {
...
PUBLIC void mint()
{
	++mcount;
	getimmed(&target, 0x1);
	if (!(immadr.data & (FORBIT | RELBIT | UNDBIT)) && (opcode_t) immadr.offset == 0x3) {
		immcount = 0x0;
		opcode = 0xCC;
	}
}
...
PUBLIC void mjcc()
{
	...
	if (sym == IDENT &&
	    ...
	    gsymptr->data & SIZEBIT && gsymptr->value_reg_or_op.op.routine == WORDOP && opcode < 0x80) {
		getsym();
		getea(&target);
		if (target.indcount >= 0x2 || target.base != NOREG)
			kgerror(REL_REQ);
		...
		if ((pass != 0 && !is8bitsignedoffset(lastexp.offset - lc - 2)) || last_pass == 1) {
			if (target.indcount >= 0x2 || target.base != NOREG)
				kgerror(REL_REQ);
...
PUBLIC void mmov()
{
	...
	if (mcount != 0x0) {
		if (target.base == NOREG && target.index == NOREG &&
		    (source.base == ALREG || source.base == AXREG || source.base == EAXREG)) {
...
PUBLIC void mtest()
{
	...
	if (source.base == NOREG) {
		if (mcount != 0x0) {
			if (target.indcount == 0x0
			    && (target.base == ALREG || target.base == AXREG || target.base == EAXREG))
				opcode = 0xA8 | segword;
			else {
				buildea(&target);
				opcode = 0xF6 | segword;
...
PRIVATE void do1altind()
{
	...
	if (sym == COMMA) {	/* offset from register */
		...
		if (byteflag) {
			if (!(lastexp.data & (RELBIT | UNDBIT)) && !is8bitsignedoffset(lastexp.offset))
		...
		if (byteflag || (!wordflag && !(lastexp.data & (FORBIT | RELBIT)) && (lastexp.offset >> 0x8) == dirpag)) {	/* direct addressing */
			...
			if (pass2 && (opcode == JSR_OPCODE || opcode == JMP_OPCODE) &&
			    !(lastexp.data & IMPBIT) && lastexp.offset + (0x81 - 0x3) < 0x101)
...
PUBLIC void mimmed()
{
	...
	if (sym != IMMEDIATE)
		...
		if (opcode >= 0x80 && ((nybble = opcode & 0xF) == 0x3 || nybble == 0xC || nybble >= 0xE))
...
PUBLIC void mlong()
{
	...
	if (pass2) {
		...
		if (!(lastexp.data & (RELBIT | UNDBIT))) {
			lastexp.offset = lastexp.offset - lc - lcjump;
			if (last_pass < 2 && !(lastexp.data & IMPBIT) && lastexp.offset + 0x81 < 0x101)
