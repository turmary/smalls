...
char *name;
{
	...
	if (mem_start == 0) {
		...
		for (;;) {
			...
			cc = read(fd, mem_start + filelength, (size_t) (memsize - filelength));
			if (cc <= 0)
				break;
			filelength += cc;
		}
...
PUBLIC void pproceof()
{
	...
	if (infiln != infil0) {
		...
		maclevel = iflevel = blocklevel = totwarn = toterr = linum = macnum = 0;
...
PUBLIC void readline()
{
	...
	if (maclevel != 0) {
		...
		for (; maclevel != 0; macpar = macstak->parameters, ++macstak, --maclevel)
