...
register struct sym_s *symptr;
{
	if ((symptr->type & LABIT ||
	     (symptr->data & IMPBIT && !(symptr->data & UNDBIT))) && ((symptr->data ^ lcdata) & (RELBIT | SEGM))) {
		error(SEGREL);
		return FALSE;
	}
	return TRUE;
...
{
	if (!(lastexp.data & UNDBIT) && (offset_t) (lastexp.offset + 0x80) >= 0x180)
		datatoobig();
}
...
PRIVATE void docomm()
{
	...
	if (checksegrel(labptr)) {
		if (labptr->type & (EXPBIT | LABIT))
			...
			if (!(labptr->type & COMMBIT) || lastexp.offset > labptr->value_reg_or_op.value)
...
unsigned char labits;
{
	...
	if (oldtype & LABIT && !(olddata & UNDBIT) && !pass)
		 ...
		 */
	{
		if ((olddata ^ lastexp.data) & (RELBIT | UNDBIT) || labptr->value_reg_or_op.value != lastexp.offset) {
			showredefinedlabel();
			return;
		}
	}
...
PRIVATE struct sym_s *needlabel()
{
	...
	if (sym != IDENT || (symptr = gsymptr)->type & (MACBIT | MNREGBIT | VARBIT)) {
		error(LABEXP);
		return NUL_PTR;
	}
	return symptr;
