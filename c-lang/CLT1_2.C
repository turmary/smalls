...
#endif
	printf("hello\cworld!\n\n");
	/*it will print hellocworld*/

	{
	FILE *fp;
	char s[100];

	fp = fopen("E:\\VC_FILE\\CLANG\\CLT1_1.C", "r");
	printf("First:\n");
	fprintf(stdout, "level;\t%d\n", fp->level);          /* fill/empty level of buffer */
	fprintf(stdout, "flags;\t0x%04x\n", fp->flags);         /* File status flags    */
	fprintf(stdout, "fd;\t%d\n", fp->fd);             /* File descriptor      */
	fprintf(stdout, "hold;\t%d\n", fp->hold);           /* Ungetc char if no buffer */
	fprintf(stdout, "bsize;\t%d\n", fp->bsize);          /* Buffer size          */
	fprintf(stdout, "*buffer;%d\n", *fp->buffer);        /* Data transfer buffer */
	fprintf(stdout, "*curp;\t%d\n", *fp->curp);          /* Current active pointer */
	fprintf(stdout, "istemp;\t%u\n", fp->istemp);         /* Temporary file indicator */
	fprintf(stdout, "token;\t%d\n", fp->token);          /* Used for validity checking */
	printf("fgets=%s\n", fgets(s, 100, fp),s);
	printf("buffer=%p\t%s\n", fp->buffer, fp->buffer);
	printf("curp=%p\t%s\n", fp->curp, fp->curp);
	printf("After Read:\n");
