        ...
        *NumberDevices *= 2;
        ...
        for (; i < *NumberDevices; i++, hidDeviceInst++) 
        {
                                             ...
                                             &deviceInfoData))
            {
                ...
                //
		fprintf(stderr, "Dev #%d: %s\n", i, functionClassDeviceData -> DevicePath);
...
--*/
{
    ...
    HidDevice -> DevicePath = malloc(strlen(DevicePath) + 2);
    ...
    if (GetDeviceInfo) 
    {
        ...
        {
		DWORD DeviceUsage = 0x100 * HidDevice->Caps.UsagePage + HidDevice->Caps.Usage;
	
		fprintf(stderr, "DeviceUsage = 0x%.4X\n", DeviceUsage);
